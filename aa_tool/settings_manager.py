...
class AppCache:
    ...
    # 循環翻譯連續幾輪沒有任何一話翻成功就停止（0＝不限，達標才停）。
    auto_translate_loop_idle_rounds: int = 3
    # 標題過濾「讀取網址時自動帶入」：起始網址一換就帶入作品名。預設關閉。
    ...
    #   gemini_api_system_prompt → API 模式送出；瀏覽器模式於 browser_use_gem=False 時也送
    gemini_api_only_prompt: str = ""
    gemini_api_system_prompt: str = ""
    # 瀏覽器模式是否使用 Gem（內建人設）：True 時瀏覽器模式「不送出」翻譯 Prompt
    # （只控制 gemini_api_system_prompt 那一段；gemini_api_only_prompt 在瀏覽器模式一律不送）。
    # 預設 True（多數使用者用 Gem）。
    ...
    def load_cache(self) -> AppCache:
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                cache.auto_translate_loop_idle_rounds = min(99, max(0, int(data.get(
                    'auto_translate_loop_idle_rounds',
                    cache.auto_translate_loop_idle_rounds))))
            except (TypeError, ValueError):
                pass
            cache.auto_translate_title_auto = bool(data.get(
