...
*   **🧪 實驗性「偵測文字右側 AA 圖並補空白」選項 (`pad_right_aa`)**: 設定 Dialog 勾選框「🧪 替換翻譯時：偵測文字右側 AA 圖並補全形空白」，持久化於 `aa_settings_cache.json` 的 `pad_right_aa`（預設 `False`）。動機：有些 AA 圖在對話文字的右邊，文字被換成較短的中文後右側 AA 會整個位移、需手動重排。`MainWindow._pad_right_aa` 透過 `_prepare_translation()` 連同 `symbol_regex_str=self.current_symbol_regex` 一起傳入底層 `apply_translation(..., pad_right_aa=, symbol_regex_str=)`；底層只在 `pad_right_aa=True` 時編譯 `symbol_regex`（None / 編譯失敗退回 `DEFAULT_SYMBOL_REGEX`），交給 `_replace_with_padding` → `_right_side_has_aa`（判定細節見上）。`_replace_with_padding` 收 `len_diff = len(original) - len(final_translated)` 參數，依正負兩向處理（內部以 `re.finditer` 手動掃描，逐個 match 重組，取代舊的 `pattern.sub`）：
        - **`len_diff > 0`（譯文較短）**：判定為「右側有 AA」時，於譯文後補 `len_diff` 個全形空白（與既有 Auto-Padding 相同公式），把右側 AA 圖推回原欄位。
        - **`len_diff < 0`（譯文較長）**：判定為「右側有 AA」**且 `pad_right_aa=True`** 時，先跳過並保留 match 緊接的收尾括號（`_TRAILING_CLOSERS`，屬文字尾巴不可吃），再吃掉其後最多 `-len_diff` 個空白（全形 `　` 或半形 ` ` 各記 1）；空白不足時能吃幾個算幾個（AA 圖仍會偏，但偏得較少），**不動 AA 圖本身字元**。此「消空白」邏輯嚴格 gated 在 `pad_right_aa` 實驗開關底下——關閉時 `len_diff < 0` 一律不處理（與舊行為一致）。
        - **`append_mode`**：`len_diff` 強制設為 0，不補也不消。
