...
*   **替換執行（行號限定）**: 原文按 ID 嚴格限定在對應行號替換（`line_idx = int(id.split('-')[0]) - 1`，1-indexed 轉 0-indexed），只修改該行，不觸及其他行。ID 無法解析時退回全行掃描（保護性 fallback）。同一原文出現於不同行時，各行依各自的翻譯 ID 獨立替換，互不干擾。`source_lines` 在替換迴圈前預先切割一次，整個迴圈共用，避免重複 split/join。替換本身利用 `re.escape(original)` 轉換為正則；補空白與否由 `_right_side_has_aa(rest, pad_right_aa, symbol_regex)`（`translation_engine.py`）判定 — `rest` 為被替換原文右側剩餘字串：含對話框線 `|`、`│`、`｜`、`┃` 時恆套用補空白後字串（向後相容、與 `pad_right_aa` 無關），否則直接替換以維護排版。`pad_right_aa=True` 時額外條件：先剝掉 `rest` 緊接的收尾括號（`」』）)】〕〉》｝}］]`）與空白，剩下的內容以 `aa_noise_ratio` 評估 AA 噪聲比例（定義同 `_local_aa_density`：半形片假名 ∪ AA 慣用標點 ∪ `symbol_regex` 命中字元），`>= _RIGHT_AA_NOISE_THRESHOLD`（0.34）時亦套用補空白後字串。
