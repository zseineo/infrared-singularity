...
*   **🧪 實驗性日文提取演算法（`experimental=True`）**: 設定 dialog 中的「實驗性日文提取演算法」選項（持久化至 cache `experimental_extraction: bool`，預設 `False`）。開啟後在**非韓文模式**下走另一條 per-line pipeline：
    ...
    2. **錨點掃描** (`_find_anchors`)：找下列任一 pattern：
       ...
       - `_NUMBER_OPTION_RE` 數字選項：全/半形數字（可帶範圍 `～`）+ `：`/`．` 分隔 (`[0-9０-９]+(?:[～~][0-9０-９]+)?[：．:.]`)，捕捉「１０：ご主人様」「１～３：ママぁ」這類安価/投票格式。命中時分數 `+3` 並列入 `strong_jp` 信號（使用者觀察：選項後通常不接 AA 圖）。
    3. **邊界擴展** (`_expand_boundary`)：從錨點向兩側貪心擴張，遇 `_VALID_CHAR_RE` 不命中字元、半形片假名 (`ｦ-ﾟ`)、AA 慣用標點 (`_AA_PUNCT_CHARS = {〔〕㌻㍉〟〝丶ヽヾ}`，**不含 `々〆`** — 兩者為合法日文用法，例「後々響く」「〆切」) 或空白即停。`_merge_overlapping` 合併相鄰/重疊區間。
...
*   **網頁解析 (`aa_tool/url_fetcher.py` → `parse_page_html()`)**: 依 URL 網域自動選擇解析器，由 `_DOMAIN_PARSERS` 對應表管理。分派策略：
    ...
    *   **blog.fc2.com (`_parse_fc2blog`)**: `div.ently_text`、**`div.entry_body`** 或 **`div.eBody`**（三種 FC2 模板擇一，容器 regex 為 `<div class="(?:ently_text|entry_body|eBody)"[^>]*>`） + `dl.relate_dl`（含 `web.archive.org` 封存版、`yaruok.blog.fc2.com` 等變體）。內文截止點：`fc2button-clap` div / `relate_dl` / `<!--/ently_text-->` / `<!--/entry_body-->` / `<!--/eBody-->` 最先出現者。**多容器掃描**：頁面可能有多個符合的 `<div>`（例：長期未更新的 FC2 blog 會在文章前插入「スポンサーサイト」廣告區塊，內含 `sita_rss` / counter `<script>` 等 cruft，也是 `<div class="ently_text">`）；故 `_parse_fc2blog` **逐一掃描所有容器**，每個各自抽取內文（移除 `<script>...</script>` 與 `<a>...</a>`、`<br>`→`\n`、`_strip_tags_keep_color`、`html.unescape`），取**第一個內文含貼文標頭行（`_POST_HEADER_RE`）的**；若全部都沒有則退回第一個容器（保守 fallback）。**`eBody` 變體**（例：`chronoyaruo.blog.fc2.com`）：內文以多個 `<div class="aa">` 容納、`<br>` 分行，貼文標頭「N 名前： …」（可能含 `<span style="color:..">` 包住 trip code）直接寫在內文中；續篇貼文另置於 `<div class="eBody" id="more">`，兩段之間無邊界標記，但因 `fc2button-clap`/`relate_dl` 終止點在其後，會被一併涵蓋進來。**`t_h`/`t_b` 變體**（例：`yaruyarach.blog.fc2.com`）：每則貼文為 `<div class="t_h">標頭</div><br><div class="t_b">內文…</div>`，標頭格式為「N ： AUTHOR  YYYY/MM/DD(曜) HH:MM:SS ID:XXX」（名稱與日期間僅以空白分隔，無 `：`／`[...]`）—— 此格式靠下方「作者名稱格式」第 4 種涵蓋。以上變體皆走 `_parse_fc2blog` 既有 flat 抽取流程（含 `_filter_color_by_author` 作者過濾），不需額外特化邏輯。**顏色保留策略（flat extraction 版）**：未指定作者時，保留所有 color span（讓站方的作者標色等原色顯示，例如 `yaruonichijou.com` 作者名稱以 `<span style="color:#008800;">` 標綠），之後以 `_cleanup_unmatched_spans` 清除因外層 `<span class="aa">` 等被剝掉開標籤後殘留的孤兒 `</span>`（舊行為是 `re.sub` 刪掉所有 color span，導致彩色文字顏色完全消失）。指定作者或啟用 author_only 時，仍走 `_filter_color_by_author`（只保留作者貼文的顏色）。**關聯清單排序自動偵測**：FC2 各站 `relate_dl` 原始順序不一致 —— 多數為「新 → 舊」（需 reverse 取得時間順序），但部分站點（例：`yaruosippu.blog.fc2.com`）原始 HTML 已是「舊 → 新」時間順序，此時 reverse 反而會讓「下一話」按鈕變成上一集。解法：以 `_is_nav_ascending_by_date()` 從標題尾端 `(YYYY/MM/DD)` 抽日期、比較相鄰配對的遞增/遞減配對數，遞增多數時保留原順序、其餘（含無日期或日期皆同的情境）退回 reverse 預設行為。
...
*   **作者名稱格式**: `_is_author_post()` 支援四種標頭格式並自動正規化空白（透過 `_extract_poster_name()` 共用解析）：
    ...
    行內切分用的 `_POST_HEADER_RE`（供 `_filter_color_by_author` 判斷貼文邊界）同樣支援這些格式；`_POSTER_NAME_RE_ALT`（與 `_POST_HEADER_RE` 第二 alternative）的名稱／日期分隔符 `(?:[：:]|\[[^\]]*\])?` 設為**可選**，配合前置 `\s*`，同時涵蓋格式 2、3、4。<br>**注意此放寬的副作用**：理論上一行若形如「N ： 任意文字 YYYY年/YYYY/...」（以數字開頭、含 4 位數年份接 `/` 或 `年`）會被誤判為標頭；實務上 AA 內文極少出現此形狀，且僅影響 `author_only` 過濾的貼文邊界切分，可接受。
...
*   **章節號碼自動偵測 (`check_chapter_number()` / `get_chapter_display()`)**: 貼上或讀取文字後，掃描前 200 字尋找話數格式，自動填入話數欄位。支援格式（依序嘗試）：`第N-N話`（複合話數，如「第5-2話」→ 話數填 `5-2`，分節編號完整保留；分隔符支援 `-－ー−ｰ`、數字支援阿拉伯／全形）、`第N話`（阿拉伯數字 / 漢數字）、`番外編N`、`後日談N`（阿拉伯／全形／漢數字；例：「エージェントやる夫の日常茶飯事　後日談3」）、`その N`（全形/半形）、**裸 `N話`（無「第」前綴）—— 僅限文本第一行**，例：「やる夫の淫らな日々　2話　きっと掌の上だった日…」。`第N-N話` 必須排在 `第N話` 與裸 `N話` 之前嘗試，否則複合話數會被裸 `N話` 規則誤抓成後半段（如「第5-2話」誤判為 `2`）。裸 `N話` 因易與內文誤命中，刻意限制在第一行；若需擴展請維持此限制。
