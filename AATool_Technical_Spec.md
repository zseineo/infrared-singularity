...
*   **狀態存儲機制**:
    *   `aa_settings_cache.json` — 暫存 UI 狀態 (原文、過濾規則、術語表、話數、預覽暫存、URL 記錄、背景/文字色、各開關狀態、作品+作者歷史 `work_history`、韓文提取模式開關 `korean_mode`、實驗性日文提取演算法開關 `experimental_extraction` 等)，確保關閉重開後不丟失資料。**正則表達式不從暫存讀取**，由 `AA_Settings.json` 管理（韓文模式啟用時改用 `DEFAULT_BASE_REGEX_KO` 常數，亦不從暫存讀取）。
...
*   **Qt 端功能**:
    ...
    *   讀取紀錄列表（最多 `fetch_history_limit` 筆，倒序顯示）+ 清除紀錄按鈕（紅）+ 搜尋框（`hist_search` `QLineEdit`，placeholder「🔍 搜尋紀錄（標題）」、`setClearButtonEnabled(True)`）。`textChanged` 觸發 `_on_history_search_changed()` 即時把 `_history_filter` 存成 lower-case 子字串，再由 `_refresh_history()` 僅對 `entry["title"]` 做 case-insensitive 子字串比對（**不比對網址**，避免使用者搜尋常見字（如 `com`、`http`）時所有紀錄全部命中）；無命中時顯示「（無符合「…」的紀錄）」提示。
...
*   **章節號碼自動偵測 (`check_chapter_number()` / `get_chapter_display()`)**: 貼上或讀取文字後，掃描前 200 字尋找話數格式，自動填入話數欄位。支援格式（依序嘗試）：`第N話`（阿拉伯數字 / 漢數字）、`番外編N`、`後日談N`（阿拉伯／全形／漢數字；例：「エージェントやる夫の日常茶飯事　後日談3」）、`その N`（全形/半形）、**裸 `N話`（無「第」前綴）—— 僅限文本第一行**，例：「やる夫の淫らな日々　2話　きっと掌の上だった日…」。裸 `N話` 因易與內文誤命中，刻意限制在第一行；若需擴展請維持此限制。
...
*   **多程序共享保護**（`aa_tool/settings_manager.py` + `aa_tool/file_lock.py`）：
    ...
    *   **細粒度 helpers**（不經過 `AppCache`，只動目標欄位）：
        *   `append_url_history(entry, max_items=50)` — 以 `url` 去重、newest-last append；寫入當下觸發（`_handle_url_fetch_request` 內），不透過 `schedule_save`。**保留戳印 meta**：若舊條目已有 `work_title` / `author` 戳印且新 entry 未提供，會自動沿用，避免重抓同一 URL 把戳印洗掉。
