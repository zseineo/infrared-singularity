...
_ERROR_POLICY_UI: list[tuple[str, list[tuple[str, str, str]]]] = [
    ...
    ("兩種模式", [
        ("reply_format", "回覆格式不符",
         "AI 不理會 prompt，回來的不是「ID|譯文」而是別的東西——最常見的是\n"
         "整篇內容摘要（「やる夫スレのログデータですね。登場人物…」）。\n"
         "這種回覆一行也替換不到，會存出一份沒翻譯的檔案，所以一律不存檔。\n"
         "判定：回覆中符合「ID|譯文」的行數不到一半。\n"
         "稍後重試＝排進待補翻列表，等下一話翻譯成功後再補翻這一話（預設）。\n"
         "跳過這一話＝記入失敗清單，直接翻下一話。"),
        ("reply_lines", "譯文行數少太多",
         "格式對，但 AI 只翻了一部分——送出 100 行、回來只有 60 行之類。\n"
         "沒回來的那些行會留著原文，所以同樣視為翻譯失敗、不存檔。\n"
         "判定：譯文的 ID 行數不到送出行數的 80%。\n"
         "稍後重試＝排進待補翻列表，等下一話翻譯成功後再補翻這一話（預設）。\n"
         "跳過這一話＝記入失敗清單，直接翻下一話。"),
        ("fetch_fail", "抓取網頁失敗",
